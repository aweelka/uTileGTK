...
proc initialise_lib {lib} {
  ...
  puts $fd "extern const char*\
            TileGtk_InitialiseSymbols_${lib}(const char *file_name);"
  puts $fc "\nconst char*\
            TileGtk_InitialiseSymbols_${lib}(const char *file_name) \{"
  puts $fc "  GModule *m = g_module_open(file_name, G_MODULE_BIND_LAZY);"
  puts $fc "  if (!m) return \"module_open\";"
